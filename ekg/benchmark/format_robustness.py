...
def _export_csv(signal: np.ndarray, fs: int, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["# sampling_rate", fs])
        writer.writerow(["lead_I"])
        for v in signal:
            writer.writerow([f"{float(v):.6f}"])


def _export_edf(signal: np.ndarray, fs: int, path: Path) -> None:
    try:
        import pyedflib
    ...
    try:
        header = {
            ...
            "physical_max": float(np.max(sig)) + 1e-6,
            "digital_min": -32768,
            "digital_max": 32767,
            "transducer": "",
            "prefilter": "",
        ...
        writer.writeSamples([sig])
    finally:
        writer.close()


def _signal_from_xml(xml_path: Path) -> tuple[np.ndarray, int]:
    """Pull a single lead and the sampling rate back out of the unified XML."""
    import base64
    ...
    if data_node is None or not data_node.text:
        raise RuntimeError(f"empty Lead/Data in {xml_path}")
...
def run_format_robustness(
    ...
    *,
    ...
    for record_id in record_ids:
        try:
            native = build_record(
                record_id, data_dir=data_dir, pre=pre, post=post, cache_dir=cache_dir,
            )
        ...
        raw_signal, ann_samples, _, fs = load_record(record_id, data_dir=data_dir or "data")
            ...
            for fmt in target_formats:
                try:
                    if fmt == "csv":
                        ...
                        _export_csv(raw_signal, fs, src)
                    elif fmt == "edf":
                        src = tmp_dir / f"{record_id}.edf"
                        _export_edf(raw_signal, fs, src)
                    else:
                        raise ValueError(f"unsupported target format {fmt!r}")
                    xml_path = parse_to_xml(src)
                    rt_signal, rt_fs = _signal_from_xml(Path(xml_path))
                except Exception as exc:  # noqa: BLE001
                ...
                rt_signal = rt_signal[: raw_signal.shape[0]]
                diff = float(np.mean(np.abs(rt_signal - raw_signal[: rt_signal.shape[0]])))
